...
5. Legacy `.dot`/`.doc` signing currently uses one pass. Agile/V3 signatures require
  their SIP-specific encoding and are not generated by this tool yet.
...
Options:
  ...
  --passes <1>                   Number of signing passes for legacy VBA files (default: 1)
...
OfficeVbaSigner "C:\Macros\report.xlsm" ^
  ...
  --alg sha256 --passes 1 --timestamp http://timestamp.digicert.com --clear --verbose
```
...
- RFC 3161 timestamps require a reachable timestamp authority (TSA) URL
